...
| `--in` | string | "" | チャンネル絞り込み。例 `#team-dev-ict-develop`（`#` 自動補完） |
...
```bash
# Slack 名で直近 7 日
slack user-activity okayama --days 7

# User ID で（解決ステップをスキップ、最速）
slack user-activity U08L3MPJB9T --days 30

# チャンネル絞り込み（# は自動補完される）
slack user-activity okayama --days 14 --in team-dev-ict-develop

# 大量取れることが見えてるなら必ず --out で context を守る
slack user-activity okayama --days 30 --count 100 --out .claude/tmp/slack/okayama-activity.json
```
...
```json
{
  ...
  "query": "from:@okayama after:2026-05-07",
